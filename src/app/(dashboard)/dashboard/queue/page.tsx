...
import {
  Calendar, CheckCircle2, AlertCircle, Clock, Send,
  Image as ImageIcon, Pencil, Plus, History, Trash2, Briefcase,
} from 'lucide-react';
...
const statusConfig = {
  ...
  cancelled: { label: 'Отменено', icon: AlertCircle, badge: 'default' as const },
} as const;
                    ...
                    <div className="group flex items-start gap-3 rounded-sm px-3 py-3 transition-base hover:bg-accent/50">
                      <div
                        className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-sm ${
                          row.status === 'processing' ? 'bg-warning-soft text-warning' : 'bg-primary-soft text-primary-soft-foreground'
                        }`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          {row.auto_delete_after_hours && (
                            <Badge variant="warning" className="shrink-0" title={`Удалится через ${row.auto_delete_after_hours}ч после публикации`}>
                              <Trash2 className="h-3 w-3" />
                              авто-удаление {row.auto_delete_after_hours}ч
                            </Badge>
                          )}
                          {(() => {
                            const placement = Array.isArray(row.ad_placements) ? row.ad_placements[0] : row.ad_placements;
                            if (!placement) return null;
                            const adv = Array.isArray(placement.advertisers) ? placement.advertisers[0] : placement.advertisers;
                            return (
                              <Badge variant="primary" className="shrink-0" title={adv?.name ? `Рекламодатель: ${adv.name}` : 'Реклама'}>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            {/* Auto-delete state badges */}
                            {row.status === 'sent' && row.auto_deleted_at && !row.auto_delete_error && (
                              <Badge variant="default" className="shrink-0" title={`Удалён ${new Date(row.auto_deleted_at).toLocaleString('ru-RU')}`}>
                                <Trash2 className="h-3 w-3" />
                                удалён авто
                              </Badge>
                            )}
                            {row.status === 'sent' && row.auto_delete_error && (
                              <Badge variant="destructive" className="shrink-0" title={row.auto_delete_error}>
                                <Trash2 className="h-3 w-3" />
                                ошибка удаления
                              </Badge>
                            )}
                            {row.status === 'sent' && row.auto_delete_at && !row.auto_deleted_at && !row.auto_delete_error && (
                              <Badge variant="warning" className="shrink-0" title={`Будет удалён ${new Date(row.auto_delete_at).toLocaleString('ru-RU')}`}>
                                <Trash2 className="h-3 w-3" />
                                удалится через {formatTimeUntil(row.auto_delete_at)}
                              </Badge>
                            )}
                            {(() => {
                              const placement = Array.isArray(row.ad_placements) ? row.ad_placements[0] : row.ad_placements;
                              ...
                              {(() => {
                                ...
                                return (
                                  <ReportLinkButton
                                    ...
                                    advertiserTelegram={adv?.telegram_username ?? null}
