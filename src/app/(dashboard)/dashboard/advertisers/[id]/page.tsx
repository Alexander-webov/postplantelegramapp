...
import {
  ArrowLeft, Briefcase, AtSign, Mail, FileText, Archive, ArchiveRestore,
...
export default async function AdvertiserDetailPage({ params }: PageProps) {
  ...
  const { data: advertiser } = await supabase
    .from('advertisers')
    .select('id, name, telegram_username, contact, notes, total_placements, total_revenue_rub, archived_at, created_at')
    .eq('id', id)
    .eq('user_id', user.id)
    .single();
  ...
  // Pull placement history for this advertiser. Batch 2 will populate this
  // — for now the table will be empty for new users, and we show a soft
  // empty-state message that explains how to start.
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      {isArchived && (
        <Card className="border-warning/30 bg-warning-soft">
          <CardContent className="flex items-start gap-3 p-4 text-sm">
            ...
            <div>
              <strong>Этот рекламодатель в архиве.</strong> Историю размещений ты по-прежнему видишь, но в активном списке его нет, и в композере он не появится. Можно вернуть из архива — кнопка справа.
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Card>
          <CardContent className="p-4">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">Размещений</div>
            <div className="mt-1 text-2xl font-semibold tabular-nums">
              {advertiser.total_placements}
            </div>
          </CardContent>
        </Card>
        <Card className="border-primary/30 bg-primary-soft/40">
          <CardContent className="p-4">
            <div className="text-xs uppercase tracking-wider text-muted-foreground">Заработано</div>
            <div className="mt-1 text-2xl font-semibold tabular-nums">
      ...
      {(advertiser.telegram_username || advertiser.contact || advertiser.notes) && (
        <Card>
          <CardContent className="space-y-2.5 p-5 text-sm">
            ...
            {advertiser.notes && (
              <div className="flex items-start gap-2">
                ...
                <div className="whitespace-pre-wrap break-words">{advertiser.notes}</div>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        {!placements || placements.length === 0 ? (
          <Card className="border-dashed bg-card/50">
            <CardContent className="px-5 py-6 text-center text-sm text-muted-foreground">
              Размещений ещё нет. В следующем обновлении добавим выбор рекламодателя в композере — тогда посты будут автоматически попадать сюда.
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardContent className="p-2">
              {placements.map((p, i) => {
                const sp = Array.isArray(p.scheduled_posts) ? p.scheduled_posts[0] : p.scheduled_posts;
                const ch = sp ? (Array.isArray(sp.channels) ? sp.channels[0] : sp.channels) : null;
                return (
                  <div key={p.id}>
                    ...
                    <div className="flex items-start gap-3 rounded-sm px-3 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 text-sm">
                          <span className="font-medium">{formatRub(p.price_rub)}</span>
                          <PlacementStatusBadge status={p.status} />
                          {p.format && <span className="text-xs text-muted-foreground">{p.format}</span>}
                        </div>
                        {ch && (
                          <div className="mt-0.5 text-xs text-muted-foreground">
                            ...
                            {typeof sp.views_latest === 'number' && (
                              <>
                                {' · '}
                                <span className="font-medium">{sp.views_latest.toLocaleString('ru-RU')}</span> просмотров
                              </>
                            )}
                          </div>
                        )}
      ...
      {!isArchived && (
        <section className="space-y-3">
          <h2 className="text-sm font-medium text-muted-foreground">Редактирование</h2>
          <Card>
...
function PlacementStatusBadge({ status }: { status: string }) {
  const map: Record<string, { variant: 'default' | 'primary' | 'success' | 'warning' | 'destructive'; label: string }> = {
