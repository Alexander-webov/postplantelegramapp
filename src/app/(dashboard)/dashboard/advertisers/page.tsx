...
export default async function AdvertisersPage() {
  ...
  const { data: rows } = await supabase
    .from('advertisers')
    .select('id, name, telegram_username, contact, total_placements, total_revenue_rub, archived_at, created_at')
    .eq('user_id', user.id)
  ...
  // Total revenue across all active advertisers
  ...
  return (
    <div className="space-y-6">
      ...
      {active.length > 0 && (
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <StatCard
            ...
            value={String(active.reduce((s, a) => s + (a.total_placements ?? 0), 0))}
          />
              ...
              >
                ...
                <div className="group flex items-center gap-3 rounded-sm px-3 py-3 transition-base hover:bg-accent/50">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {a.contact && (
                      <div className="truncate text-xs text-muted-foreground">{a.contact}</div>
                    )}
                  </div>
      ...
      {/* Archived section — collapsed details */}
      {archived.length > 0 && (
        <details className="group">
          <summary className="flex cursor-pointer items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
            ...
            <span className="text-xs text-muted-foreground/70 group-open:hidden">показать</span>
            <span className="hidden text-xs text-muted-foreground/70 group-open:inline">скрыть</span>
          </summary>
              ...
              {archived.map((a, i) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 rounded-sm px-3 py-2.5 transition-base hover:bg-accent/50">
                    ...
                    <Badge variant="default" className="shrink-0">архив</Badge>
                  </div>
                </Link>
              ))}
            </CardContent>
