    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">Telegram-каналы и боты которыми ты управляешь.</p>
        </div>
        ...
        <Card>
          <CardContent className="py-10 text-center">
            ...
            <p className="mb-4 text-muted-foreground">
              Сначала подключи бота — без него Постплан не сможет публиковать в Telegram.
            </p>
            <Button asChild>
              <Link href="/dashboard/channels/connect">Подключить первого бота</Link>
            </Button>
          </CardContent>
        </Card>
      )}
            ...
            {bots!.map((bot) => (
              <div key={bot.id} className="flex items-center gap-3 rounded-md border p-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">{bot.first_name ?? '—'}</div>
                </div>
              </div>
            ))}
          </CardContent>
          ...
          <CardContent>
            {(channels?.length ?? 0) === 0 ? (
              ...
              <div className="space-y-2">
                {channels!.map((ch) => (
                  <div key={ch.id} className="flex items-center justify-between rounded-md border p-3">
                    <div className="flex items-center gap-3">
                      <Radio className="h-4 w-4 text-muted-foreground" />
                      <div>
                        <div className="font-medium">{ch.title}</div>
                        <div className="text-xs text-muted-foreground">
                          {ch.username ? `@${ch.username}` : 'private'} · {ch.subscriber_count} подписчиков
                        </div>
                      </div>
                    </div>
                  </div>
